...
// 
//
// Related Topics 数组 前缀和 滑动窗口 👍 23 👎 0

  
package leetcode.editor.cn;
public class BestTimeToBuyAndSellStockUsingStrategy{
  public static void main(String[] args) {
       Solution solution = new BestTimeToBuyAndSellStockUsingStrategy().new Solution();
  }
...
class Solution {
    public long maxProfit(int[] prices, int[] strategy, int k) {
        ...
        for(int i = 0; i < n; i ++ ) {
            ...
            sumP[i + 1] = sumP[i] + prices[i];
        }

        long ans = s[n];
        for(int i = 0; i <= n - k; i ++ ) {
            long t = s[i] + s[n] - s[i + k] + sumP[i + k] - sumP[i + k / 2];
            ans = Math.max(ans, t);
        }
        return ans;
    }
