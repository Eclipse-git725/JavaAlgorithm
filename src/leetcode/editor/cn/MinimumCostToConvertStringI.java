...
// 
//
// Related Topics 图 数组 字符串 最短路 👍 39 👎 0
...
public class MinimumCostToConvertStringI {
    public static void main(String[] args) {
        Solution solution = new MinimumCostToConvertStringI().new Solution();
    }
    ...
    class Solution {
        public long minimumCost(String source, String target, char[] original, char[] changed, int[] cost) {
            ...
            for(int i = 0; i < source.length(); i ++ ) {
                if(dis[source.charAt(i) - 'a'][target.charAt(i) - 'a'] == INF) {
                    return -1;
                }
                ans += dis[source.charAt(i) - 'a'][target.charAt(i) - 'a'];
            }
            return ans;
        }
    }
