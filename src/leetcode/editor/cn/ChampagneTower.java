...
class Solution {
    public double champagneTower(int poured, int query_row, int query_glass) {
        double[] champagne = new double[1];  // 维护每层的香槟数量
        champagne[0] = poured;

        for(int i = 1; i <= query_row; i ++ ) {
            double[] nxt = new double[i + 1];
            for(int j = 0; j < champagne.length; j ++ ) {
                double x = champagne[j] - 1;
                if(x > 0) {
                    nxt[j] += x / 2;
                    nxt[j + 1] += x / 2;
                }
        ...
        return Math.min(champagne[query_glass], 1);
    }
}
