...
// 
//
// Related Topics 贪心 数组 动态规划 堆（优先队列） 👍 500 👎 0
...
public class MinimumNumberOfRefuelingStops {
    public static void main(String[] args) {
        ...
        solution.minRefuelStops(100, 10, new int[][]{{10, 60}, {20, 30}, {30, 30}, {60, 40}});
    }
    ...
    class Solution {
        public int minRefuelStops(int target, int startFuel, int[][] stations) {
            ...
            int maxD = startFuel;
            while(maxD < target) {
                while(cur < n && maxD >= stations[cur][0]) {
                    pq.add(stations[cur][1]);
                    cur ++;
                }
                if(pq.isEmpty()) {
                    return -1;
