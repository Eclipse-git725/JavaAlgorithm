...
public class MinimumCostHomecomingOfARobotInAGrid {
    public static void main(String[] args) {
        Solution solution = new MinimumCostHomecomingOfARobotInAGrid().new Solution();
    }
    ...
    class Solution {
        public int minCost(int[] startPos, int[] homePos, int[] rowCosts, int[] colCosts) {
            ...
            int ans = 0;
            int l = Math.min(x0, x1), r = Math.max(x0, x1);
            for(int i = l; i <= r; i ++ ) {
                ans += rowCosts[i];
            }
            l = Math.min(y0, y1);
            ...
            return ans - rowCosts[x0] - colCosts[y0];
        }
    }
