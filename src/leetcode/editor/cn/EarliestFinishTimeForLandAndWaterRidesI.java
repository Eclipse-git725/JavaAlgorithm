...
// 
//
// Related Topics 贪心 数组 双指针 二分查找 排序 👍 19 👎 0
...
public class EarliestFinishTimeForLandAndWaterRidesI{
  public static void main(String[] args) {
       Solution solution = new EarliestFinishTimeForLandAndWaterRidesI().new Solution();
  }
...
class Solution {
    ...
    public int solve(int[] landStartTime, int[] landDuration, int[] waterStartTime, int[] waterDuration) {
        ...
        for(int i = 0; i < n; i ++ ) {
            minF = Math.min(landStartTime[i] + landDuration[i], minF);
        }
