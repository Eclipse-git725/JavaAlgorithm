...
// 
//
// Related Topics 贪心 数组 数学 二分查找 堆（优先队列） 👍 51 👎 0
...
public class MinimumNumberOfSecondsToMakeMountainHeightZero {
    public static void main(String[] args) {
        Solution solution = new MinimumNumberOfSecondsToMakeMountainHeightZero().new Solution();
    }
    ...
    class Solution {
        public long minNumberOfSeconds(int mountainHeight, int[] workerTimes) {
            int maxv = 0;
            for(int v : workerTimes) {
                maxv = Math.max(maxv, v);
            }
            long l = 0, r = (long)maxv * mountainHeight * (1 + mountainHeight) / 2;

            while(l + 1 < r) {
                long mid = (l + r) / 2;
                long sum = 0;
                for(int i = 0; i < workerTimes.length; i ++ ) {
                    sum += (long)(Math.sqrt(1 + 8 * (mid / workerTimes[i])) - 1) / 2;
