...
public class MinimumDistanceBetweenThreeEqualElementsIi {
    public static void main(String[] args) {
        Solution solution = new MinimumDistanceBetweenThreeEqualElementsIi().new Solution();
    }
    ...
    class Solution {
        public int minimumDistance(int[] nums) {
            int n = nums.length;
            int[] last = new int[n + 1]; // 上一次出现的位置
            int[] last1 = new int[n + 1]; // 上上次出现的位置
            Arrays.fill(last, -2 * n);
            Arrays.fill(last1, -2 * n);

            int ans = 2 * n;
            for(int i = 0; i < n; i ++ ) {
                ans = Math.min(ans, 2 * (i - last1[nums[i]]));
                last1[nums[i]] = last[nums[i]];
                last[nums[i]] = i;
            }
            if(ans == 2 * n) {
                return -1;
