...
public class MaximumPathScoreInAGrid{
  public static void main(String[] args) {
       Solution solution = new MaximumPathScoreInAGrid().new Solution();
  }
...
class Solution {
    public int maxPathScore(int[][] grid, int k) {
        ...
        int ans = dfs(m - 1, n - 1, grid, memo, k);
        return ans < 0 ? -1 : ans;
    }

    int dfs(int i, int j, int[][] grid, int[][][] memo, int k) {
        if(i < 0 || j < 0 || k < 0) {
        ...
        int newk = grid[i][j] > 0 ? k - 1 : k;
        memo[i][j][k] = Math.max(dfs(i - 1, j, grid, memo, newk), dfs(i, j - 1, grid, memo, newk)) + grid[i][j];
        return memo[i][j][k];
    }
}
