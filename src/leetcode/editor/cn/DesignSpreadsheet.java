...
// 
//
// Related Topics 设计 数组 哈希表 字符串 矩阵 👍 16 👎 0
...
class Spreadsheet {
    ...
    public int getValue(String formula) {
        ...
        if(f[1] >= 'A' && f[1] <= 'Z') {
            ans += map.getOrDefault(formula.substring(1, i), 0);
        }else {
            ans += Integer.valueOf(formula.substring(1, i));
        }

        if(f[i + 1] >= 'A' && f[i + 1] <= 'Z') {
            ans += map.getOrDefault(formula.substring(i + 1), 0);
        }else {
            ans += Integer.valueOf(formula.substring(i + 1));
        }
        return ans;
    }
}
