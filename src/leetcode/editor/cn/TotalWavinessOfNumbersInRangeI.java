...
// 
//
// Related Topics 数学 动态规划 枚举 👍 6 👎 0

  
package leetcode.editor.cn;
public class TotalWavinessOfNumbersInRangeI{
  public static void main(String[] args) {
       Solution solution = new TotalWavinessOfNumbersInRangeI().new Solution();
  }
    ...
    public int totalWaviness(int num1, int num2) {
        ...
        for(int i = num1; i <= num2; i ++ ) {
            ...
            for(int j = 1; j < num.length() - 1; j ++ ) {
                ...
                if((ch > num.charAt(j - 1) && ch > num.charAt(j + 1)) ||
                        (ch < num.charAt(j - 1) && ch < num.charAt(j + 1))) {
                    ans ++;
                }
            }
        }
